from other_db_calls import (ground_info, tournament_info, format_info, innings_info,
overs_info, city_info, margin_runs_info, margin_wickets_info, dates_info, toss_info)
import pandas as pd
...
def get_others_df(list_other):
    ...
    if "venue" in list(list_other.keys()):
        ...
        g_var = "tournament"
    elif "format_type" in list(list_other.keys()):
        df = format_info(list_other["format_type"], gender)
        g_var = "format_type"
...
def get_fielding_text_other(field_df, format_type):
    field_text = ""
    if len(field_df):
