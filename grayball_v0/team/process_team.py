...
def process_team_recent_bat(list_other):
    ...
    if 'format_type' in list(list_other.keys()):
        ...
        format_types = get_distinct_formats_team(team_u, gender)
    ...
    for format_type in format_types:
        bat_df = batsman_df[batsman_df["match_type"] == format_type]
        bat_df = slice_with_others(bat_df, list_other)
        if len(bat_df):
            text_5_bat = recent_data_bat_team(format_type,bat_df, list_other["recent_number"])
...
def process_team_recent_ball(list_other):
    ...
    for format_type in format_types:
        bowl_df = bowler_df[bowler_df["match_type"] == format_type]
        bowl_df = slice_with_others(bowl_df, list_other)
        if len(bowl_df):
            text_5_ball = recent_data_ball_team(format_type,bowl_df, list_other["recent_number"])
...
def process_team_recent_field(list_other):
    ...
    field_stats_text = ""
    for format_type in format_types:
        field_df = get_fielding_recent_stats_team(format_type, team_u, list_other["recent_number"], gender)
        field_text = get_fielding_text_team(field_df, team_u, format_type)
...
def process_team_field(list_other):
    ...
    field_stats_text = ""
    for format_type in format_types:
        field_df = get_fielding_stats_team(format_type, team_u, gender)
        field_text = get_fielding_text_team(field_df, team_u, format_type)
        field_stats_text += field_text
