...
def get_batting_stats(odi_batt, player, format_type):
    ...
    text = f"""In the {format_type}, {player} has played {inngs} innings, taking {total_balls} balls to score {total_runs} runs.
...
def get_bowling_stats(odi_ball, player, format_type):
    ...
    text = f"""In the {format_type}, {player} has bowled in {inngs} innings, bowled {valid_balls} balls i.e. {valid_overs} overs giving away {total_bowler_runs} runs.
...
def get_fielding_text_player(field_stats):
    ...
    if len(field_stats):
        ...
        for idx, row in field_df.iterrows():
            field_text += f"In the {row['format_type']}, {row['player']} has done {row['stats'][0]} run outs and {row['stats'][1]} stumpings and took {row['stats'][2]} catches."
